...
- Widget extension 入口位于 `Sources/BirthTrackerWidget`；具体 Widget、provider、view 和 AppIntent 配置位于 `BirthTrackerPackage/Sources/BirthTrackerWidgets`，并按 `Shared`、`ContactAge`、`UpcomingBirthdays` 分目录组织。
- 面向 Widget 共享的 UI、模型和持久化常量放在 package 模块里，而不是 App-only 代码里。
