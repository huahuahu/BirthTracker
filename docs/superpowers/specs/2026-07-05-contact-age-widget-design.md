...
`ContactAgeWidget` 继续使用 `AppIntentConfiguration`。联系人选择复用 `SelectPersonIntent`，配置参数保存联系人 UUID 字符串，并通过 `WidgetPersonOptionsProvider` 从 Widget 快照提供动态候选；timeline provider 只通过 `selectedPersonID` 读取联系人 UUID，再从 Widget 快照加载扁平数据。
