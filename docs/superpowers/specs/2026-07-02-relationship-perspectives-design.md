...
产品概念上使用 `Person` 表示一个人。第一版继续使用当前代码里的 `TrackedPerson` 作为 SwiftData 存储模型；实现关系系统时不需要为了产品命名立即重命名 Swift 类型。

`Birthday` 是人持有的生日值对象。后续生日列表、提醒和年龄计算继续基于现有生日能力，只是在展示时额外叠加当前视角下的关系称谓。
...
- 是否需要导入通讯录时自动建议关系。
- 是否需要关系图可视化页面。
- Swift 类型名是否在未来从 `TrackedPerson` 迁移为 `Person`。
